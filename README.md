...
hybrid ranking, and an optional LLM layer — with **no vector database and no
hosted API**. Everything below runs on the local machine.
...
through `sentence-transformers` with GPU auto-detected; the query sequence length
is capped at 1024 tokens for several-fold faster encoding. Vectors are
...
keeps encoding cheap, the index small, and — critically — lets the corpus cover
novels that were *crawled but never downloaded*, which is why it scales to 100k+.

### Retrieval — exact cosine, no ANN

The index is one `float32` `[N × 1024]` matrix. A query becomes a single NumPy
matrix–vector product (`embeddings @ q`) — sub-millisecond and **exact** at
10k–100k rows. There is deliberately no FAISS/HNSW: at this scale an approximate
index would add a dependency and lose recall for no latency benefit. The matrix is
the index.
...
- **`like <title>`** — reuse the seed novel's *stored* vector as the query; no
  model load, instant "more like this".
- **`tags <…>`** — pure structured filter + rank, no embedding at all.

### Optional local LLM (Qwen2.5-3B)

Loaded lazily and only when requested; every method degrades gracefully (returns
its input unchanged on error):
...
`url → hash`. A rebuild re-embeds only records that are new or whose hash changed,
so updating a 10k-record corpus after editing a handful of novels costs a handful
of forward passes rather than a full re-encode. `recommend.py build --rebuild`
forces a full pass.
...
The recommender is fed and surrounded by a small set of repository-root scripts
(`python <script>.py --help` for full options) and a reading web app.
...
[`reader-app/`](reader-app/README.md) is a local React + FastAPI reader that
renders downloaded novels with offline **pinyin ruby** and **hover dictionary**
definitions in a paper-themed UI, remembers your place, and shares the
`data/reading_progress.json` bookmark with `read.py`.

```bash
~/venvs/recsys/bin/python reader-app/setup.py     # one time
python read.py --gui                              # build + open the reader
```
...
python read.py "Love U2" --gui             # 5. read it (web app or --copy to clipboard)
python report.py catalogue                 # coverage / disk-usage / chain reports
```

Bulk crawling and downloading can split work across two public IPs (direct route
+ a Windscribe tunnel) with `--windscribe` to roughly double throughput; see
`--help` on `scrape_metadata.py` / `download.py`.

### Environment & install

Run everything in one virtualenv so the editable `recsys`, `scraper`, `scripts`,
and `webnovel` packages resolve:

```bash
python3 -m venv ~/venvs/recsys
# Pick the right PyTorch build for the machine (RTX 50-series example):
...
The scraper still holds 52shuku-specific HTML/URL logic; supporting another site
means moving those rules behind a site-adapter interface while keeping the shared
download, storage, recommendation, and reading workflows.
